...
project_dir <- file.path("/scratch/chrY_varaints_in_PCa/Jenna/snp_parsing/batch1")
manifest <- file.path(project_dir, "infinium-methylationepic-v-1-0-b5-manifest-file.csv.gz") #hg19
vcf <- "/scratch/chrY_varaints_in_PCa/Jenna/germline_merge/batch1/epic_germline_b1_99_nochr.vcf.gz" #hg19
report.dir <- "/scratch/chrY_varaints_in_PCa/Jenna/snp_parsing/batch1/reports"
...
probe_annot <- probe_annot[idxs_cg, c("IlmnID", "Infinium_Design_Type", "Next_Base", "CHR", "MAPINFO", "Strand")]
...
sbe <- with(probe_annot[type1_idxs, ], GRanges(seqnames=CHR, 
                                               ranges=IRanges(start=ifelse(Strand=="F", MAPINFO-1, MAPINFO+2), width=1),
...
rest1 <- with(probe_annot[type1_idxs, ], GRanges(seqnames=CHR, 
                                                 ranges=IRanges(start=ifelse(Strand=="F", MAPINFO+2, MAPINFO-5), width=5),
                                                 strand=ifelse(Strand=="F", "+", "-"),
                                                 name=IlmnID))
...
#save(variants, 
#     file=file.path(report.dir, "epic_variants_106_b1.Rdata"))
#load(file.path(report.dir, "epic_variants_106_b1.Rdata"))
...
save(variant_table_new, variant_regions, 
     file=file.path(report.dir, "epic_variants_99_b1.Rdata"))
colSums(is.na(variant_table))
...
probe_annot <- read.csv(gzfile(manifest), skip=7, header=TRUE)
idxs_cg <- grep("^cg", probe_annot$IlmnID)
probe_annot <- probe_annot[idxs_cg, c("IlmnID", "Infinium_Design_Type", "CHR", "MAPINFO", "Strand", "Next_Base")]
rownames(probe_annot) <- probe_annot$IlmnID
# Create relevant ranges: SBE, CPG, REST
type1_idxs <- probe_annot$Infinium_Design_Type=="I"
type2_idxs <- probe_annot$Infinium_Design_Type=="II"
...
write.csv(rv_sbe, file=file.path(report.dir, "epic_SNPs_b1_99_sbe.csv"),
...
write.csv(rv_cpg, file=file.path(report.dir, "epic_SNPs_b1_99_cpg.csv"),
          row.names=FALSE, quote=FALSE)
...
write.csv(rv_rest, file=file.path(report.dir, "epic_SNPs_b1_99_body_5bp.csv"),
...
epic.variants1 <- read.csv("/scratch/chrY_varaints_in_PCa/Jenna/snp_parsing/batch1/reports/epic_SNPs_b1_99_sbe.csv", head = T)
epic.variants2 <- read.csv("/scratch/chrY_varaints_in_PCa/Jenna/snp_parsing/batch1/reports/epic_SNPs_b1_99_cpg.csv", head = T)
epic.variants3 <- read.csv("/scratch/chrY_varaints_in_PCa/Jenna/snp_parsing/batch1/reports/epic_SNPs_b1_99_body_5bp.csv", head = T)
...
write.csv(epic.snp.probes, file=file.path(report.dir, "epic_SNPs_b1_99_all.csv"),
          row.names=T, quote=F)